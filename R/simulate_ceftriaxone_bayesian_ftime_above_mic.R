...
simulate_ceftriaxone_bayesian_ftime_above_mic <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, MIC, PCTABOVEMIC, CRCLCAP, HISTORY, REGIMENS) {
  ...
  ## API call
  simulate("ceftriaxone-bayesian-ftime-above-mic", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, MIC = MIC, PCTABOVEMIC = PCTABOVEMIC, CRCLCAP = CRCLCAP, HISTORY = HISTORY, REGIMENS = REGIMENS)
}
