...
simulate_vancomycin_mc_emp_auc_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, CREATININE, MODEL, MIC, AUCPERMIC, LOADINGDOSE, TOXICITYINFO, CRCLCAP, REGIMENS) {
  ...
  ## API call
  simulate("vancomycin-mc-emp-auc-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, CREATININE = CREATININE, MODEL = MODEL, MIC = MIC, AUCPERMIC = AUCPERMIC, LOADINGDOSE = LOADINGDOSE, TOXICITYINFO = TOXICITYINFO, CRCLCAP = CRCLCAP, REGIMENS = REGIMENS)
}
