#' Meropenem » Empiric » % fT > MIC
#' 
#' Meropenem » Empiric therapy » Percent time of free concentration above MIC
#' 
#' \strong{Drug}:
#' Meropenem
#' 
...
#' 
#' @examples \dontrun{
#' simulate_meropenem_mc_emp_ftime_above_mic(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
#' }
simulate_meropenem_mc_emp_ftime_above_mic <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, CREATININE, MODEL, EDEMA, MIC, PCTABOVEMIC, LOADINGDOSE, CRCLCAP, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("meropenem-mc-emp-ftime-above-mic", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, CREATININE = CREATININE, MODEL = MODEL, EDEMA = EDEMA, MIC = MIC, PCTABOVEMIC = PCTABOVEMIC, LOADINGDOSE = LOADINGDOSE, CRCLCAP = CRCLCAP, REGIMENS = REGIMENS)
}
