...
#' 
#' @examples \dontrun{
#' simulate_tobramycin_mc_cfr_cmin_mic_ratio(PATID = "Anonymous", 
#'     AGE = 75, HEIGHT = 160, 
...
simulate_tobramycin_mc_cfr_cmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, CREATININE, CMINPERMIC, REGIMENS, MICDISTR) {
  ...
  ## API call
  simulate("tobramycin-mc-cfr-cmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, CREATININE = CREATININE, CMINPERMIC = CMINPERMIC, REGIMENS = REGIMENS, MICDISTR = MICDISTR)
}
