#' Meropenem » Bayesian » fCmin/MIC
#' 
#' Meropenem » Bayesian adaptive dosing » Free minimum concentration to MIC ratio
#' 
#' \strong{Drug}:
#' Meropenem
#' 
...
#' 
#' @examples \dontrun{
#' simulate_meropenem_bayesian_fcmin_mic_ratio(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
#' }
simulate_meropenem_bayesian_fcmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, EDEMA, MIC, CMINPERMIC, CRCLCAP, HISTORY, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("meropenem-bayesian-fcmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, EDEMA = EDEMA, MIC = MIC, CMINPERMIC = CMINPERMIC, CRCLCAP = CRCLCAP, HISTORY = HISTORY, REGIMENS = REGIMENS)
}
