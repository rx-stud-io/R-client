#' Ceftriaxone » Bayesian » fCmin/MIC
#' 
#' Ceftriaxone » Bayesian adaptive dosing » Free minimum concentration to MIC ratio
#' 
#' @section Drug:
#' Ceftriaxone
#' 
...
#' 
#' @examples \dontrun{
#' simulate_ceftriaxone_bayesian_fcmin_mic_ratio(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
#'             set = "REGIMEN")))
#' }
simulate_ceftriaxone_bayesian_fcmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, MIC, CMINPERMIC, CRCLCAP, HISTORY, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("ceftriaxone-bayesian-fcmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, MIC = MIC, CMINPERMIC = CMINPERMIC, CRCLCAP = CRCLCAP, HISTORY = HISTORY, REGIMENS = REGIMENS)
}
