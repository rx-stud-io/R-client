#' Piperacillin & Tazobactam » CFR » % fT > MIC
#' 
#' Piperacillin & Tazobactam » Cumulative Fraction of Response » Percent time of free concentration above MIC
#' 
#' \strong{Drug}:
...
#' 
#' @examples \dontrun{
#' simulate_piperacillin_tazobactam_mc_cfr_ftime_above_mic(PATID = "Anonymous", 
#'     AGE = 35, HEIGHT = 155, 
...
#' }
simulate_piperacillin_tazobactam_mc_cfr_ftime_above_mic <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, CREATININE, MODEL, PCTABOVEMIC, CRCLCAP, REGIMENS, MICDISTR) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("piperacillin-tazobactam-mc-cfr-ftime-above-mic", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, CREATININE = CREATININE, MODEL = MODEL, PCTABOVEMIC = PCTABOVEMIC, CRCLCAP = CRCLCAP, REGIMENS = REGIMENS, MICDISTR = MICDISTR)
}
