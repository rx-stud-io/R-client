...
#' 
#' @examples \dontrun{
#' simulate_vancomycin_mc_cfr_auc_mic_ratio(PATID = "Anonymous", 
#'     AGE = 75, HEIGHT = 160, 
...
simulate_vancomycin_mc_cfr_auc_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, CREATININE, MODEL, AUCPERMIC, CRCLCAP, REGIMENS, MICDISTR) {
  ...
  ## API call
  simulate("vancomycin-mc-cfr-auc-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, CREATININE = CREATININE, MODEL = MODEL, AUCPERMIC = AUCPERMIC, CRCLCAP = CRCLCAP, REGIMENS = REGIMENS, MICDISTR = MICDISTR)
}
