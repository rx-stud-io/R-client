#' Gentamicin » PTA » AUC/MIC
#' 
#' Gentamicin » Probability of Target Attainment » AUC to MIC ratio
#' 
#' \strong{Drug}:
#' Gentamicin
#' 
...
#' 
#' @examples \dontrun{
#' simulate_gentamicin_mc_pta_auc_mic_ratio(PATID = "Anonymous", 
#'     AGE = 35, HEIGHT = 155, 
...
simulate_gentamicin_mc_pta_auc_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, CREATININE, AUCPERMIC, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("gentamicin-mc-pta-auc-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, CREATININE = CREATININE, AUCPERMIC = AUCPERMIC, REGIMENS = REGIMENS)
}
