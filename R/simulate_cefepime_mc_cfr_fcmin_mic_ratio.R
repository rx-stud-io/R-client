#' Cefepime » CFR » fCmin/MIC
#' 
#' Cefepime » Cumulative Fraction of Response » Free minimum concentration to MIC ratio
#' 
#' @section Drug:
#' Cefepime
#' 
...
#'     MICDISTR = list("MICDISTR-0.0078125" = list(
#'         0), "MICDISTR-0.015625" = list(
#'         0.8), "MICDISTR-0.03125" = list(
#'         21.4), "MICDISTR-0.0625" = list(
#'         33.9), "MICDISTR-0.125" = list(
#'         12.8), "MICDISTR-0.25" = list(
#'         6), "MICDISTR-0.5" = list(
#'         3.5), "MICDISTR-1" = list(
#'         3.8), "MICDISTR-2" = list(
#'         5), "MICDISTR-4" = list(
#'         3.5), "MICDISTR-8" = list(
#'         2), "MICDISTR-16" = list(
#'         1.2), "MICDISTR-32" = list(
#'         1), "MICDISTR-64" = list(
#'         1.7), "MICDISTR-128" = list(
#'         0.8), "MICDISTR-256" = list(
#'         0.9), "MICDISTR-512" = list(
#'         1.7)))
#' }
simulate_cefepime_mc_cfr_fcmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, CREATININE, MODEL, CMINPERMIC, CRCLCAP, REGIMENS, MICDISTR) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("cefepime-mc-cfr-fcmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, CREATININE = CREATININE, MODEL = MODEL, CMINPERMIC = CMINPERMIC, CRCLCAP = CRCLCAP, REGIMENS = REGIMENS, MICDISTR = MICDISTR)
}
