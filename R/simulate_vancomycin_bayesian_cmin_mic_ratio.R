#' Vancomycin » Bayesian » Cmin/MIC
#' 
#' Vancomycin » Bayesian adaptive dosing » Total minimum concentration to MIC ratio
#' 
#' \strong{Drug}:
#' Vancomycin
#' 
...
#' 
#' @examples \dontrun{
#' simulate_vancomycin_bayesian_cmin_mic_ratio(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
simulate_vancomycin_bayesian_cmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, MIC, CMINPERMIC, TOXICITYINFO, CRCLCAP, HISTORY, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("vancomycin-bayesian-cmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, MIC = MIC, CMINPERMIC = CMINPERMIC, TOXICITYINFO = TOXICITYINFO, CRCLCAP = CRCLCAP, HISTORY = HISTORY, REGIMENS = REGIMENS)
}
