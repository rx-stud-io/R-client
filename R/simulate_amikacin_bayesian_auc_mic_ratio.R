...
simulate_amikacin_bayesian_auc_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, MIC, AUCPERMIC, HISTORY, REGIMENS) {
  ...
  ## API call
  simulate("amikacin-bayesian-auc-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, MIC = MIC, AUCPERMIC = AUCPERMIC, HISTORY = HISTORY, REGIMENS = REGIMENS)
}
