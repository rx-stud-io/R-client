...
#' 
#' @examples \dontrun{
#' simulate_amikacin_mc_cfr_cmax_mic_ratio(PATID = "Anonymous", 
#'     AGE = 75, HEIGHT = 160, 
#'     WEIGHT = 105, GENDER = "Male", 
#'     MODEL = "Saez Fernandez et al. (2019) - General ward", 
#'     CREATININE = 1, CMAXPERMIC = 8, 
#'     REGIMENS = list(list(
#'         DOSE = 400, INTERVAL = 4, 
...
simulate_amikacin_mc_cfr_cmax_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, CREATININE, CMAXPERMIC, REGIMENS, MICDISTR) {
  ...
  ## API call
  simulate("amikacin-mc-cfr-cmax-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, CREATININE = CREATININE, CMAXPERMIC = CMAXPERMIC, REGIMENS = REGIMENS, MICDISTR = MICDISTR)
}
