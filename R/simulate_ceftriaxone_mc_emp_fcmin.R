#' Ceftriaxone » Empiric » fCmin
#' 
#' Ceftriaxone » Empiric therapy » Free minimum concentration
#' 
#' @section Drug:
#' Ceftriaxone
#' 
...
#' 
#' @examples \dontrun{
#' simulate_ceftriaxone_mc_emp_fcmin(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
simulate_ceftriaxone_mc_emp_fcmin <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, CREATININE, MODEL, MIC, CMIN, LOADINGDOSE, CRCLCAP, REGIMENS) {
  ...
  ## API call
  simulate("ceftriaxone-mc-emp-fcmin", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, CREATININE = CREATININE, MODEL = MODEL, MIC = MIC, CMIN = CMIN, LOADINGDOSE = LOADINGDOSE, CRCLCAP = CRCLCAP, REGIMENS = REGIMENS)
}
