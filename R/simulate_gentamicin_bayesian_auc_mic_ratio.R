#' Gentamicin » Bayesian » AUC/MIC
#' 
#' Gentamicin » Bayesian adaptive dosing » AUC to MIC ratio
#' 
#' \strong{Drug}:
#' Gentamicin
#' 
...
#' 
#' @examples \dontrun{
#' simulate_gentamicin_bayesian_auc_mic_ratio(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
simulate_gentamicin_bayesian_auc_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, MIC, AUCPERMIC, HISTORY, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("gentamicin-bayesian-auc-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, MIC = MIC, AUCPERMIC = AUCPERMIC, HISTORY = HISTORY, REGIMENS = REGIMENS)
}
