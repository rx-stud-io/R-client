#' Voriconazole » CFR » fAUC/MIC
#' 
#' Voriconazole » Cumulative Fraction of Response » fAUC to MIC ratio
#' 
#' \strong{Drug}:
#' Voriconazole
#' 
...
#' 
#' @examples \dontrun{
#' simulate_voriconazole_mc_cfr_fauc_mic_ratio(PATID = "Anonymous", 
#'     AGE = 35, HEIGHT = 155, 
...
#' }
simulate_voriconazole_mc_cfr_fauc_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, RIFAMPICIN, SEVEREHEPATICCHOLESTASIS, AUCPERMIC, ORALORIVORKGREGIMENS, MICDISTR) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("voriconazole-mc-cfr-fauc-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, RIFAMPICIN = RIFAMPICIN, SEVEREHEPATICCHOLESTASIS = SEVEREHEPATICCHOLESTASIS, AUCPERMIC = AUCPERMIC, ORALORIVORKGREGIMENS = ORALORIVORKGREGIMENS, MICDISTR = MICDISTR)
}
