#' Tobramycin » Empiric » Cmin
#' 
#' Tobramycin » Empiric therapy » Total minimum concentration
#' 
#' @section Drug:
#' Tobramycin
#' 
...
#' 
#' @examples \dontrun{
#' simulate_tobramycin_mc_emp_cmin(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
#'             TINF = 0.5)))
#' }
simulate_tobramycin_mc_emp_cmin <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, CREATININE, MIC, CMIN, LOADINGDOSE, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("tobramycin-mc-emp-cmin", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, CREATININE = CREATININE, MIC = MIC, CMIN = CMIN, LOADINGDOSE = LOADINGDOSE, REGIMENS = REGIMENS)
}
