...
#' 
#' @examples \dontrun{
#' simulate_gentamicin_mc_pta_cmin_mic_ratio(PATID = "Anonymous", 
#'     AGE = 35, HEIGHT = 155, 
#'     WEIGHT = 55, GENDER = "Female", 
#'     MODEL = "Pai et al. (2011) - General ward", 
#'     CREATININE = 1.0638498, 
#'     CMINPERMIC = 2, REGIMENS = list(
#'         list(DOSE = 200, 
#'             INTERVAL = 6, 
#'             TINF = 0.5, 
#'             set = "REGIMEN")))
...
#' }
simulate_gentamicin_mc_pta_cmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, CREATININE, CMINPERMIC, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("gentamicin-mc-pta-cmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, CREATININE = CREATININE, CMINPERMIC = CMINPERMIC, REGIMENS = REGIMENS)
}
