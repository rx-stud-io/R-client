...
#' @param LOADINGDOSEPERKG Required loading dose per kg. Required loading dose in mg/kg, calculated for the corrected weight. Must be provided as numeric (min. 1, max. 100 mg(PE)/kg).
...
simulate_phenytoin_mc_loading_ncmin_24h <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, PHENYTOINFORM, MODEL, ALBUMIN, RENALFUNCTIONSTATUS, VALPROICACID, LOADINGDOSEPERKG, CPOSTDOSE24H) {
  ...
  assert_number(LOADINGDOSEPERKG,
    lower = 1,
    upper = 100
  )
  ...
  ## API call
  simulate("phenytoin-mc-loading-ncmin-24h", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, PHENYTOINFORM = PHENYTOINFORM, MODEL = MODEL, ALBUMIN = ALBUMIN, RENALFUNCTIONSTATUS = RENALFUNCTIONSTATUS, VALPROICACID = VALPROICACID, LOADINGDOSEPERKG = LOADINGDOSEPERKG, CPOSTDOSE24H = CPOSTDOSE24H)
}
