...
#' 
#' @examples \dontrun{
#' simulate_meropenem_mc_cfr_fcmin_mic_ratio(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
simulate_meropenem_mc_cfr_fcmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, CREATININE, MODEL, EDEMA, CMINPERMIC, CRCLCAP, REGIMENS, MICDISTR) {
  ...
  ## API call
  simulate("meropenem-mc-cfr-fcmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, CREATININE = CREATININE, MODEL = MODEL, EDEMA = EDEMA, CMINPERMIC = CMINPERMIC, CRCLCAP = CRCLCAP, REGIMENS = REGIMENS, MICDISTR = MICDISTR)
}
