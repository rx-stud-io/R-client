#' Meropenem » Empiric » fAUC/MIC
#' 
#' Meropenem » Empiric therapy » fAUC to MIC ratio
#' 
#' \strong{Drug}:
#' Meropenem
#' 
...
#' 
#' @examples \dontrun{
#' simulate_meropenem_mc_emp_fauc_mic_ratio(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
#' }
simulate_meropenem_mc_emp_fauc_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, CREATININE, MODEL, EDEMA, MIC, AUCPERMIC, LOADINGDOSE, CRCLCAP, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("meropenem-mc-emp-fauc-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, CREATININE = CREATININE, MODEL = MODEL, EDEMA = EDEMA, MIC = MIC, AUCPERMIC = AUCPERMIC, LOADINGDOSE = LOADINGDOSE, CRCLCAP = CRCLCAP, REGIMENS = REGIMENS)
}
