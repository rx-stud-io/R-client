#' Tobramycin » Bayesian » Cmin/MIC
#' 
#' Tobramycin » Bayesian adaptive dosing » Total minimum concentration to MIC ratio
#' 
#' @section Drug:
#' Tobramycin
#' 
...
#' 
#' @examples \dontrun{
#' simulate_tobramycin_bayesian_cmin_mic_ratio(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
...
#'             TINF = 0.5)))
#' }
simulate_tobramycin_bayesian_cmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, MIC, CMINPERMIC, HISTORY, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("tobramycin-bayesian-cmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, MIC = MIC, CMINPERMIC = CMINPERMIC, HISTORY = HISTORY, REGIMENS = REGIMENS)
}
