...
#' }
simulate_gentamicin_mc_emp_cmin_cmax <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, CREATININE, MIC, CMIN, CMAX, LOADINGDOSE, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("gentamicin-mc-emp-cmin-cmax", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, CREATININE = CREATININE, MIC = MIC, CMIN = CMIN, CMAX = CMAX, LOADINGDOSE = LOADINGDOSE, REGIMENS = REGIMENS)
}
