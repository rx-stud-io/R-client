#' Tobramycin » CFR » AUC/MIC
#' 
#' Tobramycin » Cumulative Fraction of Response » AUC to MIC ratio
#' 
#' @section Drug:
#' Tobramycin
#' 
...
#'         2.7), "MICDISTR-0.25" = list(
#'         10.2), "MICDISTR-0.5" = list(
#'         49.1), "MICDISTR-1" = list(
#'         23.4), "MICDISTR-2" = list(
#'         4.2), "MICDISTR-4" = list(
#'         1.9), "MICDISTR-8" = list(
...
#'         7.3)))
#' }
simulate_tobramycin_mc_cfr_auc_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, CREATININE, AUCPERMIC, REGIMENS, MICDISTR) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("tobramycin-mc-cfr-auc-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, CREATININE = CREATININE, AUCPERMIC = AUCPERMIC, REGIMENS = REGIMENS, MICDISTR = MICDISTR)
}
