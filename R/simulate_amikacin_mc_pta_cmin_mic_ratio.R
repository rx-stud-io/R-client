#' Amikacin » PTA » Cmin/MIC
#' 
#' Amikacin » Probability of Target Attainment » Total minimum concentration to MIC ratio
#' 
#' \strong{Drug}:
#' Amikacin
#' 
...
#' 
#' @examples \dontrun{
#' simulate_amikacin_mc_pta_cmin_mic_ratio(PATID = "Anonymous", 
#'     AGE = 35, HEIGHT = 155, 
...
#' }
simulate_amikacin_mc_pta_cmin_mic_ratio <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, CREATININE, CMINPERMIC, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("amikacin-mc-pta-cmin-mic-ratio", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, CREATININE = CREATININE, CMINPERMIC = CMINPERMIC, REGIMENS = REGIMENS)
}
