#' Cefepime » Bayesian » \% fT > MIC
#' 
#' Cefepime » Bayesian adaptive dosing » Percent time of free concentration above MIC
#' 
#' \strong{Drug}:
#' Cefepime
#' 
...
#' 
#' @examples \dontrun{
#' simulate_cefepime_bayesian_ftime_above_mic(PATID = "Anonymous", 
#'     AGE = 65, HEIGHT = 175, 
#'     WEIGHT = 75, GENDER = "Male", 
#'     MODEL = "Tam et al. (2003) - General ward", 
#'     MIC = 4, PCTABOVEMIC = 50, 
#'     CRCLCAP = "No cap", 
#'     HISTORY = list(list(
...
simulate_cefepime_bayesian_ftime_above_mic <- function(PATID, AGE, HEIGHT, WEIGHT, GENDER, MODEL, MIC, PCTABOVEMIC, CRCLCAP, HISTORY, REGIMENS) {
  ## check args
  assert_string(PATID)
  assert_number(AGE,
    lower = 18,
  ...
  ## API call
  simulate("cefepime-bayesian-ftime-above-mic", PATID = PATID, AGE = AGE, HEIGHT = HEIGHT, WEIGHT = WEIGHT, GENDER = GENDER, MODEL = MODEL, MIC = MIC, PCTABOVEMIC = PCTABOVEMIC, CRCLCAP = CRCLCAP, HISTORY = HISTORY, REGIMENS = REGIMENS)
}
